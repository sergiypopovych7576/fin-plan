...
export class MonthSelectorComponent implements OnInit {
  ...
  private _formMonth(): void {
    if(this.currentDate.getMonth() === this.currentMonthNumber && this.currentDate.getFullYear() === this.currentYear) {
      this.monthIsCurrent = true;
    }
    this.currentMonth = this.currentDate.toLocaleString('en-US', { month: 'long' });
    this.currentMonth = this.currentMonth.charAt(0).toUpperCase() + this.currentMonth.slice(1) + ' ' + this.currentDate.getFullYear();
  }
